...
# --- Upload Excel Files ---
cashify_file = st.file_uploader("Upload Cashify CPL Excel File", type=["xlsx"], key="cashify")
maple_file = st.file_uploader("Upload Maple CPL Excel File", type=["xlsx"], key="maple")
...
if cashify_file and maple_file:
    try:
        ...
        if not time_columns:
            ...
            df_total_tradein = df_total_tradein[
                pd.to_datetime(df_total_tradein["Month"], format='%b-%Y') >= pd.to_datetime("Sep-24", format='%b-%y')]
            ...
            fig_total_tradein = px.bar(df_total_tradein, x="Month", y=["CPL Total", "Cashify Total", "Maple Total"],
                                       ...
                                       color_discrete_map={
                                           "CPL Total": "#252529",
                                           "Cashify Total": "#10a4a0",
                                           "Maple Total": "#b6f5e3"})
            st.plotly_chart(fig_total_tradein)

            # Monthly Trade-in of Cashify
            ...
            df_comparison = df_comparison[
                pd.to_datetime(df_comparison["Month"], format='%b-%Y') >= pd.to_datetime("Sep-24", format='%b-%y')]
